...
class AnimatedPNGEncoder : public Encoder {
    ...
    public:
    AnimatedPNGEncoder(const Image& image, const std::vector<Frame>& frames, const uint32_t targetWidth, const uint32_t targetHeight)
    : Encoder(image), frames(frames), targetWidth(targetWidth), targetHeight(targetHeight) {
    }
};