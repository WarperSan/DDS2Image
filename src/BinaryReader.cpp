...
void BinaryReader::advance(size_t bytes) { position += bytes; }

BinaryReader::BinaryReader(std::span<const uint8_t> buf)
...
uint32_t BinaryReader::readUInt() {
  canRead(4);
  uint32_t value = buffer[position] | buffer[position + 1] << 8 |
                   buffer[position + 2] << 16 | buffer[position + 3] << 24;
  advance(4);
  return value;
}

int32_t BinaryReader::readInt() { return static_cast<int32_t>(readUInt()); }
